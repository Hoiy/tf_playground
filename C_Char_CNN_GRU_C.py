...
from keras.layers import MaxPooling1D, Input, GRU, LSTM, Conv1D, Conv2D, Conv2DTranspose, Dense, Flatten, Dropout, Reshape, Embedding, Concatenate
...
def create_baseline():
    ...
    x = Embedding(NUM_SYMBOL, EMBEDDING_SIZE, input_length=SEQ_LENGTH)(inp)
    
    #emb = x
    #for l in range(layer):
    #    x = Conv1D(dim[l], kernel[l], strides=strides[l], activation=activation, padding=padding, use_bias=use_bias)(x)
            
    #x = LSTM(gru_dim, activation=activation, recurrent_activation='relu', use_bias=True, dropout=0.1, recurrent_dropout=0.1)(x)
    #x = Concatenate()([emb, x])
    x = GRU(gru_dim, return_sequences=True)(x)
    x = GRU(gru_dim)(x)
    #x = Flatten()(x)
    #x = Dropout(0.2)(x)
...
model.fit_generator(
    ...
    epochs=200000,
    callbacks=[testSample(), TensorBoard(), LossHistory(), ReduceLROnPlateau()]
